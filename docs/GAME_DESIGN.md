# Dig Deeper — 게임 기획 v0.2

> v0.2: 발밑 부스터 도입(암벽 등반 삭제), 플레이 가능 프로토타입(`prototype/index.html`)
> 자동 플레이테스트로 검증한 실측치 반영. 수정 사유는 각 절의 인용 블록에 남겼다.
...
| 항목 | 값 | 비고 |
|---|---|---|
| 최대 연료 | 100 | 스킬로 최대 175 |
| 상승 속도 | 200 px/s (10타일/s) | **등속** — 가속형이 아니라 공중 정지·미세 조작 가능 |
| 연료 소모 | 36 /초 | 만탱크 ≈ 2.8초 ≈ 28타일 |
| 접지 충전 | 34 /초 (100%) | 발이 땅에 닿아 있을 때 |
| 굴뚝 충전 | 27 /초 (80%) | 좌우 벽에 닿아 **완속 하강(70 px/s)** 중 |
| 공중 충전 | 0 | 「냉각 코일」 해금 시 6 /초 |
...
> **밸런스 앵커 (프로토타입 실측):** 무보급 직하강 왕복 기준 —
> 무강화 **52m**, 중반 빌드 **79m**, 트리 완주 **187m**.
> 여기에 에어포켓·산소 결정·「회수 호흡」 보급을 더해야 심연층(420m+)에 닿는다.
> v0.1이 적어 둔 "무강화 75~85m / 완주 700m, 8~9배"는 측정 전 추정치였고,
> 실측 배율은 **약 3.6배(무보급)**다. 650m는 *보급 전제* 목표로 유지한다 —
> 이 구분이 없으면 "산소 = 제한 시간"이 후반에 무의미해진다.
...
| T4 (★ 정점) | 해당 갈래 누적 **9포인트** | 1,600 G + 코어 조각 3 | 1 |
...
> 잡아 0/1/5/9로 교정했다 — 이제 49회 구매로 4갈래 정점 4/4 + 교차 노드 4/4가 모두 열린다.
